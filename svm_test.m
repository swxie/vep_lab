...
for  k = 1 : length(train_set)
    make_dataSet(".\data\" + train_set(k) + "*", arg);
    [mean_acc, std_acc, entropy] = svm_classify(10, 0.1);
    fprintf("%s\t%f\t%f\t%f\n", train_set(k), mean_acc, std_acc,entropy);
end
for i = 1 : 3
    for j = 1 : 3
        arg.dwt_order = j;
...
%用于生成数据集的函数
function [state] = make_dataSet(name, arg)
...
function [mean_acc, std_acc, mean_entropy] = svm_classify(k, pos_ratio, nu, sigma)
